test_that("Identifying ID samples with low reconstruction accuracy", {
  ...
  sig.prop <- ExposureProportions(mutation.type = "ID",
                                  cancer.type = "Panc-Endocrine")
  sigs.to.use <- ID.sigs[, names(sig.prop), drop = FALSE]
  ...
  panc.sparse.out <-
    MAPAssignActivity(
      spectra = ID.three.spectra[, 1, drop = FALSE],
      sigs = sigs.to.use,
