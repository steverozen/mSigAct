...
stacked.pcat <- function(bg.catalog, target.catalog) {
  ...
  for (i in 1:ncol(bg.catalog)) {
    ...
    ICAMS::PlotCatalog(
      catalog = cbind(bg.catalog[ , i, drop = FALSE],
                      target.catalog[ , i, drop = FALSE]),
      upper   = (i == 1),
      xlabels = xlabels)
    
    legend("topright", 
           legend = c(params$whichnitro, "Background"),
           fill = c("grey35", "red"))
  }
}
...
min.sub
bp <- pcat1(inferred.sig, ylim = c(min.sub, max(arrow.tops) + 0.05))
...
bp <- pcat1(
  ICAMS::TransformCatalog(inferred.sig,
                          target.catalog.type = "density.signature"),
  ylim = c(min.sub, max(arrow.tops) + 0.05))
add.arrows(bp, arrow.tops, arrow.bottoms)
