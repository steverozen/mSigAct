...
#' 
#' @keywords internal
ShowExposure <- function(list.of.sig.activity, output.dir, 
                         plot.exposure.proportion = FALSE) {
  ...
  if (isTRUE(plot.exposure.proportion)) {
    output.file <- 
      file.path(output.dir, "all.samples.exposure.props.pdf")
  } else {
    output.file <- 
      file.path(output.dir, "all.samples.exposure.counts.pdf")
  }
...
#'
#' @keywords internal
WriteDistances <- function(list.of.sig.activity, output.dir) {
  if (!dir.exists(output.dir)) {
    dir.create(output.dir)
  }
  ...
  output.file <- file.path(output.dir, "all.samples.distances.csv")
...
PlotSigActivityToPdf <- function(list.of.sig.activity, 
  ...
  if (plot.all.samples.in.one.pdf) {
    ...
    file <- file.path(output.dir, "all.samples.sig.activity.pdf")
    ...
    for (sample in names(list.of.sig.activity)) {
      output.file <- file.path(output.dir, paste0(sample, ".sig.activity.pdf"))
